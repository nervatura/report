...
func createGoReport(t *testing.T) (rpt *Report) {
	...
	appendElement(rowData, "cell", IM{})
	appendElement("details", "vgap", IM{"height": 0, "page-break": true})
...
func TestXMLData(t *testing.T) {
	...
	if err := os.WriteFile("example/out/data.xml", []byte(dataXml), 0644); err != nil {
		t.Fatal(err)
	}
}

func TestJSONReport(t *testing.T) {
	json, _ := os.ReadFile(path.Join("example", "sample.json"))
	rpt := New("L")
	...
	if err = os.WriteFile("example/out/json.pdf", pdf, 0644); err != nil {
		t.Fatal(err)
	}
}
